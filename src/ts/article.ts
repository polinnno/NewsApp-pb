...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (tag) tag.textContent = article.category;

    const author = document.querySelector('.meta-author');
    if (author) author.textContent = `By ${article.source}`;
    ...
    if (time) time.textContent = '3 mins';

    const description = document.querySelector('.article-text');
    if (description) description.textContent = article.description ?? article.content;
    ...
    if (bigBanner && similar[0]) {
        renderArticleInto(bigBanner as HTMLElement, similar[0]);
    }
    ...
    verticals.forEach(column => {
        ...
        slots.forEach(slot => {
            if (similar[index]) {
                renderArticleInto(slot as HTMLElement, similar[index]);
                index++;
            }
        });
    });
